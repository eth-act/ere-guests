...
def read_catalog_versions(package: str, version_file: str) -> dict[str, str]:
    """Returns versions parsed from `version_file` generated by the `package` build script."""
    run_command(["cargo", "clean", "--package", package])
    run_command(["cargo", "build", "--package", package])
    outputs = (TARGET_DIR / "debug" / "build").glob(f"{package}-*/out/{version_file}")
    latest = max(outputs, key=lambda path: path.stat().st_mtime, default=None)
    if latest is None:
        raise RuntimeError(f"{version_file} not found after building {package}")
    versions = re.findall(r'Self::(\w+)\s*=>\s*"([^"]+)"', latest.read_text())
    if not versions:
        raise RuntimeError(f"failed to parse versions from {version_file}")
...
def read_zkvm_versions() -> dict[str, str]:
    """Returns zkVM SDK versions parsed from the `ere-catalog` build script."""
    return read_catalog_versions("ere-catalog", "zkvm_sdk_version_impl.rs")
